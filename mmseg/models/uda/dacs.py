...
def normalize(input):
    ...
    normalized_input = (input - min_input) / (range_input + 1e-30)

    return normalized_input
def convert_logits_ema(logits, last_vec, ema_weight, indication_method):
    '''The input is a detached tensor'''
    num_classes = logits.shape[1]
    current_vec = torch.zeros(num_classes)
    pred_prob, pred_label = torch.max(logits, dim=1)
    for ind in range(num_classes):
        temp_mask = (pred_label == ind)
        if indication_method == 'confidence':
            temp_pred = logits[:, ind, :, :]
        elif indication_method == 'margin':
            logits_topk = torch.topk(logits, 2, dim=1)
            logits_topk_indices = logits_topk.indices
            logits_topk_values = logits_topk.values
            
            logits_topk_values_diff = torch.abs(logits_topk_values[:,0,:,:] - logits_topk_values[:,1,:,:])
            temp_pred = logits_topk_values_diff
        elif indication_method == 'entropy':
            temp_entropy = logits*torch.log(logits+1e-20)
            entropy = torch.sum(temp_entropy, dim=1)
            temp_pred = entropy
        value = torch.sum(temp_pred * temp_mask)/(torch.sum(temp_mask)+1e-12)
        if last_vec is None:
            current_value = value
        else:
            last_value = last_vec[0, ind]
            current_value = ema_weight * last_value + (1-ema_weight) * value
        current_vec[ind] = current_value
    return current_vec

def convert_logits_raw(logits, indication_method):
    '''The input is a detached tensor'''
    num_classes = logits.shape[1]
    current_vec = torch.zeros(num_classes)
    pred_prob, pred_label = torch.max(logits, dim=1)
    for ind in range(num_classes):
        ...
        if indication_method == 'confidence':
            ...
            temp_pred = entropy
        value = torch.sum(temp_pred * temp_mask)/(torch.sum(temp_mask)+1e-12)
        current_vec[ind] = value
    return current_vec        


def compute_ratio(local_iter, start_iter, end_iter, domain):
    x = float(local_iter - start_iter)/ (end_iter - start_iter)
...
class DACS(UDADecorator):
    ...
    def __init__(self, **cfg):
        ...
        self.smoothness = 'ema' # 'raw'; 'ema'
        self.ema_weight = 0.999
        # Margin is too slow; Entropy is not validated
        self.indication_method = 'confidence' # 'confidence'; 'margin'; 'entropy'
        self.is_cut_mix = False
        
        self.src_indication_history = None
        self.pseudo_indication_history = None
        
        self.src_current_vec = None
        self.pseudo_current_vec = None

        self.src_last_vec = None
        self.pseudo_last_vec = None
    ...
    def forward_train(self,
        ...
        clean_losses,src_logits = self.get_model().forward_train(stage,
        ...
        if self.smoothness == 'raw':
            self.src_current_vec = convert_logits_raw(src_softmax.detach(), self.indication_method)[None, :]
        elif self.smoothness == 'ema':
            self.src_current_vec = convert_logits_ema(src_softmax.detach().cpu(), self.src_last_vec, self.ema_weight, self.indication_method)[None, :]
            self.src_last_vec = self.src_current_vec

        if self.src_indication_history is None:
            self.src_indication_history = self.src_current_vec
        else:
            self.src_indication_history = torch.vstack((self.src_indication_history, self.src_current_vec))

        
        src_prob, src_label = torch.max(src_softmax, dim=1)
        src_prob = normalize(src_prob)
        ...
        mix_losses,mix_logits = self.get_model().forward_train(stage,
            mixed_img,
            img_metas,
            mixed_lbl,
        ...
        if self.enable_masking and self.mask_mode.startswith('separate'):
            masked_loss,masked_logits = self.mic(self.get_model(), img, img_metas,
