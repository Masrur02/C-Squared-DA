...
data_root = '/home/soicroot/Downloads/Khan/Datasets/RELLIS'
img_norm_cfg = dict(
    mean=[123.675, 116.28, 103.53], std=[58.395, 57.12, 57.375], to_rgb=True)

source_input_size = (688, 550) # RUGD
target_input_size = (1920, 1200) # RELLIS
...
data = dict(
    ...
    train=dict(
        ...
        target=dict(
            type='RELLISDataset',
            data_root=data_root,
            img_dir='RELLIS_images/train',
            ann_dir='RELLIS_labels/train',
            pipeline=rellis_train_pipeline)),
    val=dict(
        type='RELLISDataset',
        data_root=data_root,
        img_dir='RELLIS_images/val',
        ann_dir='RELLIS_lables/train',
        pipeline=test_pipeline),
    test=dict(
        type='RELLISDataset',
        data_root=data_root,
        img_dir='RELLIS_images/test',
        ann_dir='RELLIS_labels/train',
        pipeline=test_pipeline))
